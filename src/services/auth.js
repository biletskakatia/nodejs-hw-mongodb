...
const createSession = () => {
    ...
    const accessTokenValidUntil = new Date(Date.now() + accessTokenLife);
    const refreshTokenValidUntil = new Date(Date.now() + refreshTokenLife);
    return {
        accessToken,
        refreshToken,
...
export const sendResetToken = async (email) => {
    ...
    const sendResetPassTemplatePath = path.join(
        TEMPLATES_DIR, 'send-reset-email.html'
    );
    const templateSource = (
        await fs.readFile(sendResetPassTemplatePath)
    ).toString();
